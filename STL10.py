...
if __name__ == '__main__':
    ...
    transform_stl = transforms.Compose([transforms.ToTensor()])
    dataset = torchvision.datasets.STL10(root='../data', download=True, transform = transform_stl)
    print(len(dataset))
    
    cal_dataset, test_dataset = torch.utils.data.random_split(dataset, [2500, 2500])
    ...
    #######################################    
    alpha = args.alpha
    print(f"Experiment--Data : STL10, Model : {model_name}, Score : THR, Predictor : SplitPredictor, Alpha : {alpha}")
    score_function = THR()
