...
if __name__ == '__main__':
    ...
    transform_food101 = transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor()])
    dataset = torchvision.datasets.Food101(root='../data', download=True, transform = transform_food101)
    
    cal_dataset, test_dataset = torch.utils.data.random_split(dataset, [37875, 37875])
    ...
    #######################################    
    alpha = args.alpha
    print(f"Experiment--Data : Food101, Model : {model_name}, Score : THR, Predictor : SplitPredictor, Alpha : {alpha}")
    score_function = THR()
